/*
  # Data Plans Categories Management

  1. New Tables
    - No new tables created
  
  2. Changes
    - Add is_popular flag to data_plans table if it doesn't exist
    - Create or replace get_data_usage_leaderboard function with fixed user_id reference
  
  3. Security
    - No security changes
*/

-- Add is_popular column to data_plans table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'data_plans' AND column_name = 'is_popular'
  ) THEN
    ALTER TABLE data_plans ADD COLUMN is_popular BOOLEAN DEFAULT FALSE;
    CREATE INDEX IF NOT EXISTS data_plans_popular_idx ON data_plans(is_popular);
  END IF;
END $$;

-- Create or replace the function with fixed user_id reference
