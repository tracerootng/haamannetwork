/*
  # Update data usage leaderboard function to return top 10 users

  1. Changes
    - Modify the `get_data_usage_leaderboard` function to return top 10 users instead of 5
    - Keep all other functionality the same
    - Ensure proper security and privacy protections

  2. Security
    - Function remains accessible to all authenticated users
    - User identities are still protected (only partial names shown)
    - Only successful transactions are counted
*/

-- Drop the existing function
DROP FUNCTION IF EXISTS get_data_usage_leaderboard(text);

-- Create updated function to get data usage leaderboard with 10 users
